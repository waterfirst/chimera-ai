...
## v1.0 — 법률 도우미 서비스 v2 (2026-03-29)
- **첫 번째 독립 서비스 배포**: 법률 도우미 API + 프론트엔드
- **korean-law-mcp 통합**: npm 패키지 64개 법률 도구 CLI 래퍼
- **백엔드 (Flask)**: 11개 API 엔드포인트
  - 법령 검색 (법제처 API 프록시)
  - 판례 검색 (CLI + 직접 API 2중 폴백)
  - 법령 원문 조회 (MST 기반)
  - 법률 용어 사전 (13개 내장 + CLI 확장)
  - 일상용어 → 법률용어 변환 (11개 매핑)
  - 통합 검색, Q&A (Gemini AI), 구독, 통계
  - XSS 방지 (html.escape), Rate Limit (10초)
  - 검색 로깅 (SQLite)
- **프론트엔드**: 탭 네비게이션, 판례/용어사전 섹션, XSS 안전 렌더링
- **3중 폴백 아키텍처**: korean-law-mcp CLI → 법제처 직접 API → 로컬 큐레이션
- GitHub: waterfirst/korean-law-service
- **의미**: 키메라 AI의 첫 공개 서비스 — 분석/생성을 넘어 실용 서비스 단계 진입
