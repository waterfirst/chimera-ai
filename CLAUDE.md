...
4. 섹터 분석 — 업종별 히트맵 + 바차트
5. **섹터별 매수/매도 시그널** — 10대 섹터(반도체/AI전력/자동차/조선/방산/바이오/2차전지/금융/에너지/소비재)별 매수🟢/관망🟡/매도🔴 판정. 근거: 모멘텀 순위 변동 + 외국인 수급 + 실적 전망
6. **NS 자본흐름 분석 (Navier-Stokes Capital Flow)** — 시장의 자금 흐름을 유체역학으로 해석:
   - **층류(Laminar) vs 난류(Turbulent) 판정**: 변동성(VIX/V-KOSPI) + 섹터간 자금 이동 속도 + 외국인 수급 방향성으로 레이놀즈 수 추정. 층류=추세 추종 유효, 난류=추세 무효화·현금 비중 확대
   - **유속 벡터**: 돈이 어디서 어디로 흐르는지 (예: 방산→반도체, 채권→주식, 국내→해외)
   - **압력 구배**: 금리·환율·유가가 만드는 자금 압력 방향
   - **와류(Vortex) 감지**: 특정 섹터에 자금이 갇혀 회전하는 버블 징후
   - 차트: 섹터간 자금흐름 Sankey 다이어그램 또는 화살표 차트 (ggplot2로 구현)
7. TACO 모델 분석 — Trump-Adjusted Capital Oscillator 6대 팩터
8. 리스크 요인 — 과열 지표 대시보드
9. 투자 전략 — 시나리오별(강세/횡보/약세) 탭 패널
10. 다음 주 전망 — 경제 캘린더 + 실적 발표 일정

### 필수 차트 (최소 9개, 모두 `#| lightbox: true`)
1. 미국 지수 일간/월간 수익률 (patchwork 조합)
2. KOSPI 일별 추이 + 거래대금
3. 시가총액별 수익률 비교 (양극화)
4. 업종별 히트맵 + 바차트
5. **섹터별 매수/매도 시그널 차트** — 10대 섹터 점수(0~100) + 🟢🟡🔴 색상 코딩
6. **NS 자본흐름 차트** — 섹터간 자금 이동 화살표/Sankey + 층류/난류 게이지
7. TACO 리스크 팩터 롤리팝 차트
8. 과열 경고 지표 바차트
9. 포트폴리오 배분 도넛 차트
